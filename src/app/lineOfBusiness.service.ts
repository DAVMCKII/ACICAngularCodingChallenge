...
export class LineOfBusinessService {
  ...
  private insuranceQuotesUrl = 'api/recentQuotes';  // URL to web api
  ...
  getAllQuotes(): Observable<InsuranceQuote[]> {
    const url = this.insuranceQuotesUrl;
    return this.http.get<InsuranceQuote[]>(url)
      .pipe(
        tap(_ => this.log('fetched inurance quotes')),
        catchError(this.handleError<InsuranceQuote[]>('getBusinessQuotes', []))
      );
  }

  getBusinessQuotes(businessID: number): Observable<InsuranceQuote[]> {
    const url = `${this.insuranceQuotesUrl}/?lineOfBusiness=${businessID}`;
    return this.http.get<InsuranceQuote[]>(url)
      .pipe(
        tap(_ => this.log('fetched inurance quotes')),
        catchError(this.handleError<InsuranceQuote[]>('getBusinessQuotes', []))
